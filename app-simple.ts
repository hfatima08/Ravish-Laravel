...
import "./rich-content";
import { alertDisabledFeature, getDefaultStatusLabel } from "../lib/provisioning";
import { ItemsView } from "./items";
import { TagsView } from "./tags-view-simple";
...
import { Events } from "@padloc/core/src/events";
import { setFeedbackForm, closeFeedbackForm } from "../lib/feedback-form";
import { clearDesiredOutcomeToast } from "../lib/desired-outcome";
...
export class App extends ServiceWorker(StateMixin(AutoSync(ErrorHandling(AutoLock(Routing(LitElement)))))) {
    ...
    async handleRoute(
        ...
        if (page === "oauth" || page === "passwordless") {
            ...
            if (page === "passwordless") {
                try {
                    await loadLanguage(locale);
                } catch (e) {
                    // Failed to load language, so we'll fallback to default (English)
                }
                pendingAuthData = stringToBase64(JSON.stringify({ code, email, type: "passwordless" }));
            } else {
                pendingAuthData = stringToBase64(JSON.stringify({ code, email }));
            }

            this.go("start", { pendingAuthData, pendingAuthState: state }, true);
            return;
        }

        if (!app.state.loggedIn) {
            if (["login", "signup"].includes(page) && !params.authToken) {
    ...
    async handleFeedbackBtn() {
        await app.log(Events.FeedbackFormOpen, { source: this._page });
        setFeedbackForm();
    }
    ...
    static styles = [
        ...
        css`
            ...
            .expired-banner {
                ...
                font-size: var(--font-size-small);
                position: relative;
                padding: max(var(--inset-top), 0.5em);
                margin-bottom: calc(-1 * var(--inset-top));
                display: flex;
            ...
            .expired-banner pl-button:hover {
                opacity: 0.9;
            }
            ...
            .feedback-float {
                ...
                box-shadow: 0px 0.5px 2.5px 0px rgba(0, 0, 0, 0.3), 0px 0px 0px 0.5px rgba(0, 0, 0, 0.05);
                cursor: pointer;
            }
                ...
                app.offline
                    ...
                    ? html`
                          <div class="expired-banner">
                              ...
                              <pl-button class="slim rounded" @click=${() => this._openRenewURL(provisioning)}>
            ...
            <div class="main">
                ...
                <div class="wrapper">
                    ...
                    <div class="views">
                        ...
                        <pl-tags-view ?hidden=${this._page !== "tags"}></pl-tags-view>
                    ...
                    <button class="feedback-float" @click=${this.handleFeedbackBtn}>${$l("Share Feedback")}<button>
                </div>
    ...
    async stateChanged() {
        ...
        if (provisioning && [ProvisioningStatus.Unprovisioned].includes(provisioning.status)) {
            const choice = await alert(
                html`<pl-rich-content .content=${provisioning.statusMessage} .type="plain"></pl-rich-content>`,
                {
                    icon: null,
                    width: "auto",
                    options: provisioning.actionLabel ? [provisioning.actionLabel!, $l("Dismiss")] : [$l("Dismiss")],
                    type: "info",
                    title: provisioning.statusLabel,
            ...
            if (choice === 0 && provisioning.actionLabel) {
                const params = new URLSearchParams();
                const id = provisioning.metaData?.oauth?.userInfo?.user.id;
                params.set("id", id);
                try {
                    const codeResponse = await this.app.api.getMAPasswordlessCode();
                    params.set("code", codeResponse.code);
                } catch (e) {}

                openExternalUrl(process.env.PL_APPLINK_PURCHASE_KEEP + "&" + params.toString());
            }
            await app.logout({ method: "forced", reason: provisioning.statusLabel });
            this.go("start");
    ...
    updated(changes: Map<string, any>) {
        ...
        if (this.app.state.openedInApp) {
            document.body.classList.toggle("theme-dark", theme === "dark");
            document.body.classList.toggle("theme-in-app-dark", theme === "dark");
            document.body.classList.toggle("theme-in-app-light", theme === "light");
        } else {
            document.body.classList.toggle("theme-dark", theme === "dark");
    ...
    async _openRenewURL(provisioning: AccountProvisioning) {
        const url = provisioning.actionUrl;
        if (!url) return;
        ...
        try {
            const codeResponse = await app.api.getMAPasswordlessCode();
            params.set("code", codeResponse.code);
        } catch (e) {}

        openExternalUrl(process.env.PL_APPLINK_RENEW + "&" + params.toString());

        app.log(Events.ClickRenew, {
            selected_cta: provisioning.actionLabel,
            route_to: provisioning.actionUrl,
        });
        app.postMessage("renew-clicked");
    }
