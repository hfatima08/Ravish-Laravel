...
export class TagsView extends Routing(StateMixin(LitElement)) {
    ...
    render() {
        if (this.selectedTag) {
            // Show selected tag view
            return html`
                <div class="selected-tag-header">
                    <div class="back-button" @click=${() => router.go("tags")}>
                        <pl-icon icon="arrow-left"></pl-icon>
                    </div>
                    <pl-icon icon="tag"></pl-icon>
                    <h2>${this.selectedTag}</h2>
                </div>
                <div class="content">
                    <p>Items with tag "${this.selectedTag}" would be displayed here.</p>
                    <p>This is a placeholder - you would integrate with your existing items list component.</p>
                </div>
            `;
        }
